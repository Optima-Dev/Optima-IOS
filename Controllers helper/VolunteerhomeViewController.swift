...
class VolunteerHomeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func configureTableView() {
        ...
        tableView.backgroundColor = .clear // تأكد من أن الخلفية شفافة
        ...
        UserService.shared.fetchCurrentUser { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .success(let user):
                    // تحديث واجهة المستخدم بالاسم الحقيقي
    ...
    private func loadData() {
        ...
        FriendService.shared.fetchFriends { [weak self] result in
            ...
            case .success(let friends):
                ...
                DispatchQueue.main.async {
                    self?.tableView.reloadData() // تحديث الجدول فورًا
                }
            case .failure(let error):
        ...
        // توليد رقم عشوائي محليًا (إذا لم يكن endpoint موجودًا)
        group.enter()
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return friends.isEmpty ? 1 : friends.count // عرض رسالة إذا كانت القائمة فارغة
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard !friends.isEmpty else {
            // عرض خلية رسالة فارغة
